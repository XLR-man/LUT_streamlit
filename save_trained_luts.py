...
parser.add_argument("--epoch", type=int, default=321, help="epoch to start training from")
parser.add_argument("--model_dir", type=str, default="LUTs/paired/fiveK_480p_3LUT_sm_1e-4_mn_0_sRGB", help="path to save model")
...
LUT1.load_state_dict(LUTs["0"])
LUT2.load_state_dict(LUTs["1"])
LUT3.load_state_dict(LUTs["2"])
...
f = open('visualization_lut/onlysm_LUT_%d_1.txt'%opt.epoch,'a')
...
f.close()
f = open('visualization_lut/onlysm_LUT_%d_2.txt'%opt.epoch,'a')
...
f.close()
f = open('visualization_lut/onlysm_LUT_%d_3.txt'%opt.epoch,'a')
